...
function logStuff(open, cameFrom, gScore, fScore) {
  let open2 = JSON.stringify(open);
  let cameFrom2 = JSON.stringify(cameFrom);
  let gScore2 = JSON.stringify(gScore);
  let fScore = JSON.stringify(fScore);
  // console.debug(`Open: ${o1}`)
  // console.debug(`Open: ${open2}`)
  console.debug(`Open: ${open2}
cameFrom: ${cameFrom2}
gScore: ${gScore2}
fScore: ${fScore2}`);
...
function aStar(start, goal, h) {
  ...
  while (open.length > 0) {
    ...
    if (current == goal) {
      console.log("IS GOAL");
      logStuff(open, cameFrom, gScore, fScore);
    ...
    console.log(`${current} Neighbors: ${JSON.stringify(neighbors)}`);
...
function call1() {
  // return aStar([0, 1], [3, 2], dist);
  o1 = aStar([0, 1], [3, 2], dist);
  return o1;
}
